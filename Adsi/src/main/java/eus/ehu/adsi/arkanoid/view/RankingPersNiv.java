...
public class RankingPersNiv extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RankingPersNiv frame = new RankingPersNiv();
	 ...
	 * @throws SQLException 
	 */
	public RankingPersNiv() throws SQLException, JSONException {
		...
		JLabel lblRankingPersonalPor = new JLabel("Ranking personal por niveles");
		lblRankingPersonalPor.setForeground(Color.WHITE);
		lblRankingPersonalPor.setFont(new Font("Tahoma", Font.PLAIN, 16));
		lblRankingPersonalPor.setBounds(25, 24, 210, 29);
		contentPane_1.add(lblRankingPersonalPor);
		
		JButton btnNewButton_1 = new JButton("Volver");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RankingPersAbs v4 = null;
				try {
					v4 = new RankingPersAbs();
		...
		JSONObject j = Controlador.getControlador().obtenerRankPN();
		int cont = 0;
		while (cont < j.length()) {
